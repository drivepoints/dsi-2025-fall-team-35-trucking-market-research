# """
# This function takes the data df and calculates the DQS, adds it as a column,
# and returns the df.
# """
...
def calc_completeness(df):
    ...
    missing_vals = ["", " ", "NA", "N/A", "None", None]
    completeness = df[KEY_FIELDS].apply(lambda col: col.isin(missing_vals).mean())
...
def calc_timeliness(df, reference_date=None):
    ...
    if "mcs150_date" in df.columns:
        mcs = pd.to_datetime(
            df["mcs150_date"].replace("None", pd.NA), format="%d-%b-%y", errors="coerce"
        )
        if mcs.notna().any():
    ...
    if "add_date" in df.columns:
        adds = pd.to_datetime(
            df["add_date"].replace("None", pd.NA), format="%d-%b-%y", errors="coerce"
        )
        if adds.notna().any():
    ...
    if "recent_mileage_year" in df.columns:
        mileage_years = pd.to_numeric(
            df["recent_mileage_year"], errors="coerce"
        ).replace(0, pd.NA)
...
if __name__ == "__main__":
    ...
    results = run_monthly_metric(
        "../data/transportation_data_20250917_222245.parquet", "2025-09"
    )
